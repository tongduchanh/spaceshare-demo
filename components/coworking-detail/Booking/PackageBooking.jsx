...
class PackageBooking extends Component {
  ...
  componentDidMount() {
    if (this.props.flexibleSubscription) {
      this.setState({
        flexibleSubscription: this.props.flexibleSubscription?.data?.results,
      })
    }
    if (this.props.coworking) {
      this.setState({
        coworking: this.props.coworking.data,
      })
    }
  }
  componentDidUpdate(prevProps) {
    ...
    if (
      prevProps.flexibleSubscription !== this.props.flexibleSubscription &&
      this.props.flexibleSubscription
    ) {
      this.setState({
        flexibleSubscription: this.props.flexibleSubscription?.data?.results,
      })
  ...
  subscriptionPlaceholder = () => {
    const subscriptionBookingError = this.props.subscriptionBookingError
    const flexibleSubscription = this.state.flexibleSubscription
    let message = ''
    if (subscriptionBookingError) {
      message = subscriptionBookingError.message
    }
    if (flexibleSubscription && flexibleSubscription.length == 0) {
                ...
                {flexibleSubscription && flexibleSubscription.length > 0 ? (
                  ...
                  <React.Fragment>
                    {!profile.data ? (
                      ...
                      <React.Fragment>
                        {subscriptionBookingError && (
                          <li>
                            <span>
                              {subscriptionBookingError.message}.{` `}
...
const mapStateToProps = (state) => {
  return {
    ...
    flexibleSubscription: state.flexibleSubscription.subscriptionBooking,
    subscriptionBookingError:
      state.flexibleSubscription.subscriptionBookingError,
  }
}
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withTranslation('common')(withRouter(PackageBooking)))
